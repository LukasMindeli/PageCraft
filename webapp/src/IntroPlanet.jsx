...
export default function App() {
  ...
  // 🎵 аудио живёт в App, чтобы НЕ обрывалось
  const audioRef = useRef(null);
  const musicSrc = new URL("./assets/intro/intro.mp3", import.meta.url).href;
  ...
  return (
    <div className="container">
      {/* аудио должно быть тут, чтобы продолжало играть после ухода Intro */}
      ...
      {!entered && (
        <IntroPlanet
          onEnter={async () => {
            await startMusic();   // play по клику пользователя (через Intro)
            setEntered(true);     // скрываем интро
          }}
        />
      )}
